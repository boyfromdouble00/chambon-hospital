/* ══════════════════════════════════════════
   참본 정형외과 — script.js
   GSAP ScrollTrigger + Snap Navigation
   ══════════════════════════════════════════ */
...
/* Button click → GSAP-powered snap transition to section */
document.querySelectorAll('a[href^="#"]').forEach(a => {
  a.addEventListener('click', (e) => {
    ...
    const scrollTop = window.scrollY + targetRect.top - 80;

    /* GSAP-powered snap — swift, elegant deceleration */
    gsap.to(window, {
...
/* Auto-float — cards drift like equipment on a turntable */
...
hero.addEventListener('mousemove', (e) => {
  ...
  gsap.to(heroBg, { x: mx * -30, y: my * -30, duration: 1, ease: 'power2.out' });
  gsap.to(cardRight, {
    x: mx * 120, y: my * 120,
    rotateY: mx * 28, rotateX: -my * 22, rotate: mx * -6, z: 40,
    duration: 0.8, ease: 'power2.out',
  });
  gsap.to(cardLeft, {
    x: mx * -90, y: my * -100,
    rotateY: mx * -22, rotateX: my * 18, rotate: mx * 8, z: 25,
    duration: 0.9, ease: 'power2.out',
  });
  gsap.to(heroContent, { x: mx * 30, y: my * 30, duration: 1.3, ease: 'power2.out' });
});
...
/* Scroll parallax */
gsap.to(heroBg, {
  y: '10%', scale: 1.06, ease: 'none',
  scrollTrigger: { trigger: '.hero', start: 'top top', end: 'bottom top', scrub: 1.3 },
});
gsap.to(cardRight, {
  y: '-18%', x: '-8%', rotate: -2, rotateY: 6, ease: 'none',
  scrollTrigger: { trigger: '.hero', start: 'top top', end: 'bottom top', scrub: 0.8 },
});
gsap.to(cardLeft, {
  y: '20%', x: '8%', rotate: 3, rotateY: -5, ease: 'none',
...
ScrollTrigger.create({
  trigger: '#stats', start: 'top center', once: true,
  onEnter: () => {
    document.querySelectorAll('.stat-number').forEach(el => {
      const target = parseInt(el.dataset.target, 10);
      gsap.to(el.closest('.stat-item'), { opacity: 1, duration: 0.3 });
      gsap.fromTo(el, { textContent: 0 }, {
        textContent: target, duration: 2.2, ease: 'power2.out',
        snap: { textContent: 1 }, roundProps: 'textContent',
      });
    });
  },
});
...
/* ══════════════════════════════════════════
   ...
   ══════════════════════════════════════════ */
['#staff', '#equipment', '#consultation'].forEach(id => {
  gsap.fromTo(`${id} .split-image img`,
    { clipPath: 'inset(0 50% 0 50%)', scale: 1.1 },
    {
      clipPath: 'inset(0 0% 0 0%)', scale: 1, duration: 1.4, ease: 'power4.inOut',
      scrollTrigger: { trigger: id, start: 'top bottom-=100', once: true },
    }
  );
});
/* Staff: feature-list left slide */
...
/* Equipment: feature-list right slide */
...
/* ══════════════════════════════════════════
   ...
   ══════════════════════════════════════════ */
document.querySelectorAll('.treatment-card').forEach((card, i) => {
  const fromLeft = i % 2 === 0;
  gsap.fromTo(card,
    { opacity: 0, x: fromLeft ? -70 : 70, scale: 0.88 },
    {
      opacity: 1, x: 0, scale: 1, duration: 0.75, ease: 'power3.out',
      scrollTrigger: { trigger: '.treatment-grid', start: 'top bottom-=40', once: true },
      delay: i * 0.1,
    }
  );
  /* 3D tilt hover */
  card.addEventListener('mousemove', (e) => {
    ...
    gsap.to(card, { rotateY: x * 10, rotateX: -y * 10, boxShadow: '0 20px 56px rgba(0,0,0,0.13)', duration: 0.4, ease: 'power2.out' });
  });
  card.addEventListener('mouseleave', () => {
    gsap.to(card, { rotateY: 0, rotateX: 0, boxShadow: '0 2px 20px rgba(0,0,0,0.06)', duration: 0.6, ease: 'power2.out' });
  });
});

/* ══════════════════════════════════════════
   REHAB — Full-section pin + scale parallax
   ══════════════════════════════════════════ */
ScrollTrigger.create({
...
gsap.fromTo('.full-width-image', { scale: 1 }, {
  scale: 1.12, ease: 'none',
  scrollTrigger: { trigger: '.full-image-section', start: 'top top', end: 'bottom bottom', scrub: 0.5 },
});
...
gsap.fromTo('.full-image-content .section-title span',
  { opacity: 0, y: 50 }, { opacity: 1, y: 0, duration: 0.8, stagger: 0.2, ease: 'power3.out',
...
/* ══════════════════════════════════════════
   ...
   ══════════════════════════════════════════ */
document.querySelectorAll('#info .info-card').forEach((card, i) => {
  const xVals = [0, -60, 60];
  gsap.fromTo(card,
    { opacity: 0, y: 40, x: xVals[i] || 0 },
    {
      opacity: 1, y: 0, x: 0, duration: 0.7, ease: 'power3.out',
      scrollTrigger: { trigger: '#info .info-grid', start: 'top bottom-=50', once: true },
      delay: i * 0.1,
    }
  );
});
...
/* ══════════════════════════════════════════
   GLOBAL PARALLAX — [data-speed]
   ══════════════════════════════════════════ */
document.querySelectorAll('[data-speed]').forEach(el => {
  const speed = parseFloat(el.dataset.speed) || 0.5;
  gsap.to(el, {
    y: () => speed * (window.innerHeight * 0.18), ease: 'none',
    scrollTrigger: {
      trigger: el.closest('section') || el.parentElement,
      start: 'top bottom', end: 'bottom top', scrub: 1 / speed,
    },
  });
});